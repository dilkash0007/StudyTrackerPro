...
const initializeLocalStorage = () => {
  ...
  if (!localStorage.getItem(KEYS.STUDY_SESSIONS)) {
    localStorage.setItem(
      KEYS.STUDY_SESSIONS,
      JSON.stringify(sampleStudySessions)
    );
  }
  ...
  if (!localStorage.getItem(KEYS.FLASHCARD_DECKS)) {
    localStorage.setItem(
      KEYS.FLASHCARD_DECKS,
      JSON.stringify(sampleFlashcardDecks)
    );
  }
...
export const taskService = {
  getTasks: (userId: number): Promise<Task[]> => {
    return new Promise((resolve) => {
      const tasks = getItem<Task[]>(KEYS.TASKS, []).filter(
...
export const studySessionService = {
  getStudySessions: (userId: number): Promise<StudySession[]> => {
    return new Promise((resolve) => {
      const sessions = getItem<StudySession[]>(KEYS.STUDY_SESSIONS, []).filter(
...
export const eventService = {
  getEvents: (userId: number): Promise<Event[]> => {
    return new Promise((resolve) => {
      const events = getItem<Event[]>(KEYS.EVENTS, []).filter(
...
export const bookService = {
  getBooks: (userId: number): Promise<Book[]> => {
    return new Promise((resolve) => {
      const books = getItem<Book[]>(KEYS.BOOKS, []).filter(
...
export const noteService = {
  getNotes: (userId: number): Promise<Note[]> => {
    return new Promise((resolve) => {
      const notes = getItem<Note[]>(KEYS.NOTES, []).filter(
...
export const flashcardDeckService = {
  getFlashcardDecks: (userId: number): Promise<FlashcardDeck[]> => {
    return new Promise((resolve) => {
      const decks = getItem<FlashcardDeck[]>(KEYS.FLASHCARD_DECKS, []).filter(
  ...
  createFlashcardDeck: (
    deck: Omit<FlashcardDeck, "id" | "userId" | "createdAt">,
    userId: number
...
export const flashcardService = {
  getFlashcards: (deckId: number): Promise<Flashcard[]> => {
    return new Promise((resolve) => {
      const flashcards = getItem<Flashcard[]>(KEYS.FLASHCARDS, []).filter(
...
export const settingsService = {
  getSettings: (userId: number): Promise<Settings | null> => {
    return new Promise((resolve) => {
      const settings =
        getItem<Settings[]>(KEYS.SETTINGS, []).find(
          (s) => s.userId === userId
        ) || null;
      resolve(settings);
    });
...
export const statsService = {
  getStats: (
    ...
    return new Promise((resolve) => {
      ...
      const studySessions = getItem<StudySession[]>(
        KEYS.STUDY_SESSIONS,
        []
      ...
      const focusScore = Math.min(
        100,
        Math.floor((totalMinutes / 3000) * 100)
      ).toString();
  ...
  > => {
    return new Promise((resolve) => {
      ...
      const flashcardDecks = getItem<FlashcardDeck[]>(
        KEYS.FLASHCARD_DECKS,
        []
